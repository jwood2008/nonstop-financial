...
Open the SQL Editor: https://supabase.com/dashboard/project/mgavuqjhqflzixbtvvig/sql/new

Paste each script below in order and click **Run**. All scripts are idempotent
(safe to re-run). `teams.sql` already includes everything from
`analytics-hardening.sql`, so that file is not needed separately.

After running all three: promote your managers in **Analytics → Users**, and
the signup manager dropdown + per-team analytics scoping go live.
...
## 4. Single-role pipeline ([supabase/single-role.sql](supabase/single-role.sql)) — run after teams.sql
...
## 5. Content editing is admin-only ([supabase/admin-only-content.sql](supabase/admin-only-content.sql))
...
## 6. Weekly Training + team chat ([supabase/weekly-training.sql](supabase/weekly-training.sql)) — run last
