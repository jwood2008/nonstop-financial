# Tab 5 — My Team analytics (managers drill into one member)

Paste after Tabs 3 and 4. Idempotent — safe to re-run.

Same as: `supabase/my-team.sql`

- `analytics_scope()` now lets a Manager pass one member's id to see that
  person's individual analytics (only members of their own team — anyone
  else's id is rejected by the database)
- `my_team()` — the manager's team list that powers the "My Team" panel
